...
from django.db.models import Sum, Prefetch, F, QuerySet
...
def ordered_products_per_customer() -> str:
    orders = Order.objects.select_related('customer').prefetch_related(
        Prefetch('orderproduct_set', queryset=OrderProduct.objects.select_related(
            'product__category'))
    ).order_by('id')
...
def available_product_with_price_gt_3() -> QuerySet:
